...
public class BlockLayoutIndexedFloatSupplier implements Supplier<IndexedFloats>
{
  ...
  private final CompressionFactory.FloatEncodingReader baseReader;

  public BlockLayoutIndexedFloatSupplier(
      int totalSize, int sizePer, ByteBuffer fromBuffer, ByteOrder order,
      CompressionFactory.FloatEncodingReader reader,
  ...
  public IndexedFloats get()
  {
    ...
    if (powerOf2) {
      // this provide slightly better performance than calling the FloatsEncodingReader.read, probably because Java
      // doesn't inline the method call for some reason. This should be removed when test show that performance
      // of using read method is same as directly accessing the floatbuffer
      if (baseReader instanceof FloatsFloatEncodingReader) {
  ...
  {
    final CompressionFactory.FloatEncodingReader reader = baseReader.duplicate();
