...
public class CompressedFloatsIndexedSupplier implements Supplier<IndexedFloats>
{
  ...
  private final CompressionFactory.FloatEncodingFormat encoding;
  ...
  CompressedFloatsIndexedSupplier(
      ...
      CompressionFactory.FloatEncodingFormat encoding
  )
  {
  ...
  {
    return buffer.remaining() + 1 + 4 + 4 + 1 + (encoding == CompressionFactory.LEGACY_FLOAT_ENCODING_FORMAT ? 0 : 1);
  }
  ...
  public void writeToChannel(WritableByteChannel channel) throws IOException
  {
    ...
    if (encoding == CompressionFactory.LEGACY_FLOAT_ENCODING_FORMAT) {
  ...
  public static CompressedFloatsIndexedSupplier fromByteBuffer(ByteBuffer buffer, ByteOrder order)
  {
    ...
    if (versionFromBuffer == LZF_VERSION || versionFromBuffer == version) {
      ...
      CompressionFactory.FloatEncodingFormat encoding = CompressionFactory.LEGACY_FLOAT_ENCODING_FORMAT;
      if (versionFromBuffer == version) {
        byte compressionId = buffer.get();
        if (CompressionFactory.hasEncodingFlag(compressionId)) {
          encoding = CompressionFactory.FloatEncodingFormat.forId(buffer.get());
