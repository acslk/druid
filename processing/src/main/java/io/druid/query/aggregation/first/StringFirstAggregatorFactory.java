...
public class StringFirstAggregatorFactory extends AggregatorFactory
{
  public static final Comparator VALUE_COMPARATOR = new Comparator()
  {
    ...
    {
      return ((SerializablePair<Long, String>) o1).rhs.compareTo(((SerializablePair<Long, String>) o2).rhs);
    }
  };
  ...
  private static final byte CACHE_TYPE_ID = 25;
  ...
  {
    return new StringFirstAggregatorFactory(name, name, maxLength)
    {
      @Override
      public Aggregator factorize(ColumnSelectorFactory columnFactory)
      {
        return new StringFirstCombiningAggregator(
            name,
      ...
      public BufferAggregator factorizeBuffered(ColumnSelectorFactory columnFactory)
      {
        return new StringFirstCombiningBufferAggregator(
            columnFactory.makeObjectColumnSelector(name),
            maxLength
        );
      }
  ...
  {
    return Arrays.<AggregatorFactory>asList(new StringFirstAggregatorFactory(fieldName, fieldName, maxLength));
  }
